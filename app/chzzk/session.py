...
ACCESS_TOKEN = os.environ["CHZZK_ACCESS_TOKEN"]  # Bearer 토큰

def create_session_url():
    headers = {
        "Authorization": f"Bearer {ACCESS_TOKEN}",
    }
    resp = requests.get(f"{OPENAPI_BASE}/open/v1/sessions/auth", headers=headers)
...
def subscribe_chat(session_key: str):
    ...
    resp.raise_for_status()
    print("subscribe response:", resp.text)
...
def run_simple_chat_listener(channel_id: str):
    ...
    subscribe_chat(session_key["value"], channel_id)
