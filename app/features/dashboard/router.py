...
async def save_command(
    ...
    success = await DashboardService(db).save_command(
        session["channel_id"],
        payload.command,
        payload.response,
    ...
    if not success:
        raise HTTPException(status_code=400, detail="명령어를 저장할 수 없습니다.")
...
async def save_greeting(
    ...
    success = await DashboardService(db).save_greeting(
        session["channel_id"],
        payload.keyword,
        payload.response,
    )
    if not success:
        raise HTTPException(status_code=400, detail="인사말을 저장할 수 없습니다.")
    return {"status": "success"}
