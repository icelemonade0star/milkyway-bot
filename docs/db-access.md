...
**운영 서버의 실제 PostgreSQL은 docker 컨테이너가 아닙니다.**
호스트에 직접 설치된 PostgreSQL(`127.0.0.1:5432`, IPv4 전용)이 진짜 운영 DB이고, **SSH 터널을 거쳐야만 접속**됩니다.

과거 `docker-compose.yml`에 `db` 서비스(`postgres:13` 컨테이너, `milkywaybot-db-1`)가 있었지만, 실제로 앱이 사용하지 않는 것으로 확인되어(2026-07-11 기준 테이블이 하나도 없는 빈 DB였음) **제거했습니다**(2026-07-11). 만약 옛 커밋이나 백업에서 이 서비스가 다시 보이더라도 `docker compose exec db psql`로 확인하면 항상 잘못된(비어있는) 결과를 보게 되니 되살리지 마세요 — 진짜 DB는 아래 방법으로만 접근합니다.
